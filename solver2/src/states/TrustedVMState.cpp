...
namespace slv2
{
    ...
    bool TrustedVMState::decide_to_write(SolverContext& context)
    {
        if(test_vectors_completed(context) && test_matrices_completed(context)) {
            //uint8_t trusted_idx = context.generals().take_decision( vsh
            //    context.trusted(),
        ...
        //uint8_t trusted_idx = context.generals().takeUrgentDecision( vsh
        //    cnt_matr_recv, // !!! actual trusted responses
        //    context.blockchain().getHashBySequence(context.round() - 1));
        return false; //(Decision::BecomeWrite == parse_generals_decision(context, trusted_idx));
    }
