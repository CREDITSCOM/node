...
void Transport::processNetworkTask(const TaskPtr<IPacMan>& task, RemoteNodePtr& sender) {
  ...
  switch (cmd) {
    ...
    case NetworkCommand::SSLastBlock:
      gotSSLastBlock(task, node_->getBlockChain().getLastSequence(), node_->getBlockChain().getLastHash(), node_->getBlockChain().canBeTrusted());
      break;
    case NetworkCommand::SSSpecificBlock: {
      try {
        cs::RoundNumber round = 0;
        iPackStream_ >> round;
        gotSSLastBlock(task, round, node_->getBlockChain().getHashBySequence(round), node_->getBlockChain().canBeTrusted());//TODO:
      }
