...
Result DefaultStateBehavior::onBlock(SolverContext& context, csdb::Pool& block, const cs::PublicKey& /*sender*/) {
  ...
  if (g_seq == awaiting_seq) {
    ...
    if (block.verify_signature()) {
      ...
      // дл¤ Writer'а вопрос решен автоматически на уровне Node (он не получает блок вообще) и на уровне WriteState (он
      // переопредел¤ет пустой метод onBlock()), а вот дл¤ Main (CollectState) ситуаци¤ не очень удобна¤, приходитс¤ не
      // делать здесь отправку хэша. я переопределил методы onBlock() в NormalState & TrustedState, где по возврату
      // значени¤ Finish выполн¤ю отправку хэша полученного блока (т.е. получилось некоторое дублирование одинакового
      // кода в наследниках). CollectSate и прочие не переопредел¤ют метод OnBlock(), соотвественно блок сохран¤ют, но
      // не отправл¤ют хэш обратно.
      return Result::Finish;
    }
    else {
      if (Consensus::Log) {
