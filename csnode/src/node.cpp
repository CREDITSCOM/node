...
void Node::getPacketHashesRequest(const uint8_t* data, const std::size_t size, const cs::RoundNumber round, const cs::PublicKey& sender) {
  ...
  cslog() << "NODE> Requested packet hashes: " << hashes.size();
...
void Node::getPacketHashesReply(const uint8_t* data, const std::size_t size, const cs::RoundNumber round, const cs::PublicKey& sender) {
  ...
  cslog() << "NODE> Hashes reply got packets count: " << packets.size();
...
void Node::getCharacteristic(const uint8_t* data, const size_t size, const cs::RoundNumber round, const cs::PublicKey& sender) {
  ...
  istream_ >> characteristic.mask;
  istream_ >> poolMetaInfo.sequenceNumber;
  istream_ >> signature;
...
void Node::retriveSmartConfidants(const cs::Sequence startSmartRoundNumber, cs::ConfidantsKeys& confs) const {
  ...
  if (table == nullptr) {
    ...
    cslog() << "___[" << startSmartRoundNumber << "] = [" << tmpPool.sequence() << "]: " << tmpKeys.size();
    for (auto& e : tmpKeys) {
...
void Node::sendStageOne(cs::StageOne& stageOneInfo) {
  ...
  csmeta(cslog) << "Round: " << roundNumber_ << "." << cs::numeric_cast<int>(subRound_)
...
void Node::getStageOne(const uint8_t* data, const size_t size, const cs::PublicKey& sender) {
  ...
  if (confidant != sender) {
    csmeta(cslog) << "Stage-1 of " << getSenderText(confidant) << " sent by " << getSenderText(sender);
  }
  else {
    csmeta(cslog) << "Stage-1 of T[" << static_cast<int>(stage.sender) << "], sender key ok";
  }
...
void Node::getSmartStageOne(const uint8_t* data, const size_t size, const cs::RoundNumber rNum, const cs::PublicKey& sender) {
  ...
  csmeta(cslog) << "Sender: " << static_cast<int>(stage.sender)
...
void Node::getSmartStageTwo(const uint8_t* data, const size_t size, const cs::RoundNumber rNum, const cs::PublicKey& sender) {
  ...
  if (!cscrypto::VerifySignature(stage.signature, confidant, bytes.data(), bytes.size())) {
    cslog() << "NODE> Stage Two from T[" << static_cast<int>(stage.sender) << "] -  WRONG SIGNATURE!!!";
    return;
  }
...
void Node::getSmartStageThree(const uint8_t* data, const size_t size, const cs::RoundNumber rNum, const cs::PublicKey& sender) {
  ...
  if (!cscrypto::VerifySignature(stage.signature, confidant, bytes.data(), bytes.size())) {
    cslog() << "Stage Two from T[" << static_cast<int>(stage.sender) << "] -  WRONG SIGNATURE!!!";
    return;
  }
...
void Node::getRoundTable(const uint8_t* data, const size_t size, const cs::RoundNumber rNum, const cs::PublicKey& sender) {
  ...
  cs::Conveyer& conveyer = cs::Conveyer::instance();
  getCharacteristic(istream_.getCurrentPtr(), istream_.remainsBytes(), conveyer.currentRoundNumber(), sender);
...
void Node::getRoundTableRequest(const uint8_t* data, const size_t size, const cs::RoundNumber rNum, const cs::PublicKey& requester) {
  ...
  cslog() << "NODE> get request for next round info after #" << rNum << " from T[" << cs::numeric_cast<int>(requesterNumber) << "]";
...
void Node::onRoundStart(const cs::RoundTable& roundTable) {
  ...
  line1 << " ROUND " << roundNumber_ << "." << cs::numeric_cast<int>(subRound_) << " ";
...
void Node::sendHashReply(const csdb::PoolHash& hash, const cscrypto::PublicKey& respondent) {
  csmeta(csdebug);
  if (myLevel_ != NodeLevel::Confidant) {
    csmeta(cswarning) << "Only confidant nodes can send hash reply to other nodes";
    return;
  }
...
void Node::getHashReply(const uint8_t* data, const size_t size, cs::RoundNumber rNum, const cs::PublicKey& sender) {
  ...
  if (std::find(confidants.cbegin(), confidants.cend(), sender) == confidants.cend()) {
    csmeta(cswarning) << "The message of WRONG HASH was sent by false confidant!";
    return;
  }

  if (!cscrypto::VerifySignature(signature, solver_->getPublicKey(), hash.to_binary().data(), hash.size())) {
    csmeta(cswarning) << "The message of WRONG HASH has WRONG SIGNATURE!";
    return;
  }

  ++badHashReplyCounter;
  if (badHashReplyCounter > confidants.size() / 2) {
    csmeta(cswarning) << "This node really have not valid HASH!!! Removing last block from DB and trying to syncronize";
