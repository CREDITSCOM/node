...
void BlockChain::testCachedBlocks() {
    ...
    while (!cachedBlocks_.empty()) {
        ...
        if ((*firstBlockInCache).first == lastSeq) {
            cslog() << kLogPrefix << "store block #" << WithDelimiters(lastSeq) << " from cache";
            ...
            csdebug() << kLogPrefix << "Stop store block from cache. Next blocks in cache #" << (*firstBlockInCache).first;
            break;
        }
    }
}
