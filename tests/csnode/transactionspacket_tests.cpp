...
TEST(TransactionsPacket, addTransactions) {
    ...
    const size_t randomTransactionsCount = cs::Random::generateRandomValue<size_t>(3, 30);
    const size_t startInnerID = cs::Random::generateRandomValue<size_t>(1, 2789);
...
TEST(TransactionsPacket, signaturesSerialization) {
    ...
    const size_t count = cs::Random::generateRandomValue<size_t>(0, maxTransactionsCount);
...
TEST(TransactionPacketHash, fromBinary) {
    ...
    transaction.set_currency(1);
    transaction.set_amount(csdb::Amount(10000, 0));
    transaction.set_innerID(cs::Random::generateRandomValue<int64_t>(1, 2789));
